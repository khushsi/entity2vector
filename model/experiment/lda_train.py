...
def preprocess_corpus(documents):
    if os.path.exists(lda_home_path+'yelp_10class_review.mm'):
        ...
        dictionary.save(lda_home_path+'yelp_10class_review.dict')  # store the dictionary, for future reference
...
if __name__ == '__main__':
    ...
    print('Loading X_idx and Y of interest')
    X_idx, Y = prepare_dataset(dp, lda_xy_path)
    X_yelp_idx  = [dp.idx2prod[x].lower().strip() for x in X_idx]
    print('Loading documents from yelp dataset')
    X_text      = load_review_text(X_yelp_idx)
